...
parser.add_argument("--target_len", type=int, default=None, help="限制左手生成序列长度")
...
# ========== 逐 token 采样生成 ==========
@torch.no_grad()
def sample_generate(model, src, bos_id, eos_id, pad_id, max_len=8000, temperature=1.0,target_len=None):
    model.eval()
...
generated_tokens = sample_generate(model, src_tensor, bos_id=BOS_ID, eos_id=EOS_ID,
                                   pad_id=PAD_ID, max_len=MAX_LEN, temperature=TEMPERATURE,target_len=args.target_len)
