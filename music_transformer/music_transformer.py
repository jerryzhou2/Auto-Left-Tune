...
if __name__ == "__main__":
    ...
    model = Seq2SeqTransformer(vocab_size=410, max_len=5000)
    checkpoint = torch.load(r"autodl-tmp/5-24.pt",
                            map_location="cuda" if torch.cuda.is_available() else "cpu")
    model.load_state_dict(checkpoint['model_state_dict'])
    train(model, train_loader, val_loader,num_epochs=50,ckpt_path=r"autodl-tmp/event/5_27_finetuning.pt",
          plt_pth=r"autodl-tmp/event/5_27_finetuning.png",resume=False)
